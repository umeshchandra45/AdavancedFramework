...
/**
 * Handles various scenarios of element presence on DOM but, may or may not be displayed yet.
 * 
 */
public class ElementPresentOnDOMChecker {
    private WebDriver driver;
    private Waits wait;

    ElementPresentOnDOMChecker(WebDriver driverInstance) {
    ...
    /**
     * Returns true if element presence on DOM but, may not be displayed yet. <b>Note:</b>This does
     * not poll. Use this if your element should be present immediately.
     * 
     ...
     * @return
     */
    public boolean byCustomPolling(By locator, int timeOutInSeconds) {
        try {
            wait.waitUntilElementTobeDisplayed(locator, timeOutInSeconds);
            return true;
        } catch (Exception e) {
            return false;
        }
