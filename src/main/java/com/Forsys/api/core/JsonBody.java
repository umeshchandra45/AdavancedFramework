...
public class JsonBody
    ...
    public JsonBody(String template, Map<String, Object> params) {
        ...
        for (Map.Entry<String, Object> e : params.entrySet()) {
            ...
            src = src.replace(MID_START_SYMBOL + e.getKey() + MID_END_SYMBOL,
                              MID_START_SYMBOL + TO_BE_REPLACED_START + e.getKey() + TO_BE_REPLACED_END +
                                                                              MID_END_SYMBOL);
        }
        ...
        for (Map.Entry<String, Object> e : params.entrySet()) {
            src = src.replace(MID_START_SYMBOL_QUOTE + TO_BE_REPLACED_START + e.getKey() + TO_BE_REPLACED_END +
                              MID_END_SYMBOL_QUOTE,
                              "\"" + convertObject(e.getValue()) + "\"");
            src = src.replace(
                              MID_START_SYMBOL + TO_BE_REPLACED_START + e.getKey() + TO_BE_REPLACED_END +
                              MID_END_SYMBOL,
                              convertObject(e.getValue()));
    ...
    public String get(String jPath) throws NoSuchFieldException {
        try {
            ...
            throw new NoSuchFieldException(
                                           String.format("Cannot find [%s] in body: [%s] \n %s",
                                                         jPath,
                                                         getBodyString(),
                                                         e.getMessage()));
        }
    ...
    private String preProcessSrc(String src) {
        String processedSrc = src.replaceAll(Pattern.quote(String.format("\"%s", TEMPLATE_START_SYMBOL)),
                                             MID_START_SYMBOL_QUOTE);
        processedSrc = processedSrc.replaceAll(Pattern.quote(String.format("%s\"", TEMPLATE_END_SYMBOL)),
                                               MID_END_SYMBOL_QUOTE);
        processedSrc = processedSrc.replaceAll(Pattern.quote(String.format("%s", TEMPLATE_START_SYMBOL)),
                                               MID_START_SYMBOL);
        processedSrc = processedSrc.replaceAll(Pattern.quote(String.format("%s", TEMPLATE_END_SYMBOL)), MID_END_SYMBOL);
        return processedSrc;
    }
    ...
    private JsonNode removeEmptyJsonNodes(JsonNode root, JsonNode template) throws IOException {
        ...
        if (root.isObject()) {
            ...
            while (fls.hasNext()) {
                ...
                if (!rootChild.getFieldNames().hasNext() && rootChildSerialized.equals(templateChildSerialized) &&
                    (rootChildSerialized.contains(MID_START_SYMBOL) || rootChildSerialized.contains(
                                                                                                    MID_START_SYMBOL_QUOTE))) {
                    nodeToRemove.add(fieldName);
                } else {
                    removeEmptyJsonNodes(rootChild, templateChild);
                    if ("{}".equalsIgnoreCase(writer.writeValueAsString(rootChild)) || "[]".equalsIgnoreCase(writer
                                                                                                                   .writeValueAsString(rootChild))) {
                        nodeToRemove.add(fieldName);
                    }
                }
